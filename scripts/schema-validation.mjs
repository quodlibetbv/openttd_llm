...
function resolveReference(rootSchema, reference) {
  if (!reference.startsWith("#/")) {
    throw new Error(`Only local schema references are supported in Phase 03: ${reference}`);
  }
...
function validateSchemaNode(schema, schemaPath, errors) {
  ...
  if (typeof schema.$ref === "string" && !schema.$ref.startsWith("#/")) {
    errors.push(`${schemaPath}: Phase 03 contracts may only use local references`);
  }
